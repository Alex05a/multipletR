...
<!-- This vignette shares its content with the package README (README.Rmd).
     The figure paths use ../man/figures/ because the vignette lives in a
     subfolder. -->
...
multiplets with read-count thresholds that assume a roughly balanced
...
<p align="center">
<img src="../man/figures/figure1_problem.png" alt="The problem: human-dominated cells are misclassified as multiplets in imbalanced PDX data." width="500"/>
</p>
...
`r BiocStyle::CRANpkg("Seurat")` package or
`r BiocStyle::Biocpkg("SingleCellExperiment")`, depending on which object type
you use.
...
See [make-data.R](../inst/scripts/make-data.R) for how this bundled PC65 example
was created.
...
these thresholds label many predominantly single-species barcodes as multiplets,
...
the same idea as tools like `r BiocStyle::Githubpkg("chris-mcginnis-ucsf/DoubletFinder", "DoubletFinder")`.
...
inspect the calls first and then filter manually on `multipletR_class`. This
gives full control over what is removed:

```{r manual-remove, eval = FALSE}
# Annotate only (the default), then filter manually on multipletR_class
seu <- remove_multiplets(seu, res, remove = FALSE)
seu_clean <- seu[, seu$multipletR_class != "Multiplet"]

# The same pattern works for a SingleCellExperiment
sce <- remove_multiplets(sce, res, remove = FALSE)
sce_clean <- sce[, sce$multipletR_class != "Multiplet"]
```
...
diverge. This lets the multiplet region adapt to each dataset.
