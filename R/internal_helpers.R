...
test_bimodality <- function(x, alpha = 0.05, min_cells_for_test = 30) {
  ...
  if (length(x) < min_cells_for_test) {
    return(list(is_bimodal = FALSE, p_value = NA_real_, dip_statistic = NA_real_,
  ...
  tryCatch({
    ...
    list(is_bimodal = FALSE, p_value = NA_real_, dip_statistic = NA_real_,
...
find_mode <- function(x) {
  ...
  tryCatch({
    ...
    if (is.null(mode_value) || length(mode_value) == 0) return(NA_real_)
...
should_stop_expanding <- function(current_metrics, previous_metrics,
  ...
  if (!is.null(previous_metrics) &&
    ...
    if (overlap_drop > overlap_drop_threshold) {
      return(list(stop = TRUE, reason = paste0("Overlap dropped ", round(overlap_drop, 1),
                                               "% from previous")))
    }
  }
  if (!is.null(previous_metrics) &&
    ...
    if (mode_diff_increase > mode_diff_increase_threshold) {
      return(list(stop = TRUE, reason = paste0("Mode diff increased ", round(mode_diff_increase, 2),
                                               " from previous")))
    }
  }
